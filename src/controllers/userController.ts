...
const changePassword = async (req: Request, res: Response) => {
  try {
    if (!req.body.currentPassword || !req.body.newPassword || !req.body.newPasswordAgain) {
      return res.json({ message: "Need to fill in form password" });
    }
...
//OK-KO
const getHomePageUser = (req: Request, res: Response) => {
  ...
  if (req.currentUser.getIsAdmin()) {
    ...
    req.currentUser.getHomePageUser(req).then((userData: any[]) => {
      ...
      res.render("user.ejs", { userData: userData, loginUser: loginUser });
    })
      .catch((err: any) => {
        res.status(400).json({
          message: "fail",
    ...
    req.currentUser.getHomePageUser(req).then((userData: any) => {
      ...
      res.render("userNotAdmin.ejs", { userData: userData, loginUser: loginUser });
    })
      .catch((err: any) => {
        res.status(400).json({
          message: "fail",
...
//OK-KO
const getUsersByEmail = (req: Request, res: Response) => {
